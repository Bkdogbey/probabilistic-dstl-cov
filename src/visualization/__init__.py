...
def _draw_env(ax, env):
    """Draw obstacles, goal, and bounds onto ax."""
    if env.bounds:
    ...
    for obs in env.obstacles:
        ...
        ax.add_patch(patches.Rectangle(
            ...
            edgecolor="darkred", facecolor="red", alpha=0.3, zorder=3,
        ))
    ...
    if env.goal:
        ...
        ax.add_patch(patches.Rectangle(
            (xlo, ylo), xhi - xlo, yhi - ylo,
            edgecolor="green", facecolor="lightgreen", alpha=0.4, zorder=2,
        ))
    ...
    for t in range(0, T + 1, ellipse_every):
        ...
        if np.all(np.isfinite(S_xy)):
            ax.add_patch(_cov_ellipse(
                mu[t, :2], S_xy, n_std=2.0,
                facecolor="steelblue", alpha=0.15, edgecolor="steelblue", linewidth=0.8,
            ))
