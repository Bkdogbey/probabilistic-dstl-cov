...
class DoubleIntegrator(BaseDynamics):
    ...
    Noise:   sigma_w on velocity channels only (position is integrated, not noisy)
    """

    def __init__(self, dt: float = 0.1, u_max: float = 2.0, sigma_w: float = 0.1, device: str = "cpu"):
    ...
    def _build_matrices(self, sigma_w: float) -> None:
        ...
        # Noise on velocity channels only: D = sigma_w * diag([0,0,1,1])
